...
export async function GET(request: NextRequest) {
  ...
  const padron = SCHOOLS.reduce(
    (acc, s) => ({
      total: acc.total + s.voters.directivos + s.voters.docentes + s.voters.asistentes,
    ...
    { total: 0, directivos: 0, docentes: 0, asistentes: 0 },
  );
  ...
  votes.total = votes.directivos + votes.docentes + votes.asistentes;
